...
app.get("/toys", async (c) => {
  ...
  if (query.name) {
    data = data.filter((toy) =>
      toy.name.toLowerCase().includes(query.name.toLowerCase()),
    );
  }
  if (query.tag) {
    data = data.filter((toy) =>
      toy.tags.some((tag) => query.tag.includes(tag)),
    );
  }
  if (query.priceMin) {
