...
app.put("/products/:id", async (c) => {
  const id = Number(c.req.param("id"));
  const product = (await c.req.json()) as Product;
  const index = products.findIndex((p) => p.id === id); // เก็บค่าเป็น index นั้นๆที่ต้องกับ id จาก parameter
  if (index === -1) {
    return c.json({ message: "Product not found" }, 404);
  }
  products[index] = { ...product, id: products[index].id }; // id: คือการระบุ id ที่ต้องการ
...
app.patch("/products/:id/status", async (c) => {
  ...
  const index = products.findIndex((p) => p.id === id); //findIndex หาไม่เจอจะเป็น -1
  if (index === -1) {
    return c.json({ message: "Product not found" }, 404);
  }
